...
const getCurrentSituation = tool({
  ...
  execute: async ({ timeWindowHours }) => {
    try {
      const reports = await $fetch(`/api/reports?hours=${timeWindowHours}&limit=30`)
      return { reports, asOf: new Date().toISOString() }
    } catch {
      return { reports: [], asOf: new Date().toISOString(), note: 'Report data unavailable' }
    }
  },
})
...
const submitReport = tool({
  ...
  execute: async (params) => {
    ...
    try {
      ...
      return { status: 'submitted', ...result as object }
    } catch (e: any) {
...
const analyzeResourceOrSector = tool({
  ...
  execute: async ({ subject, subjectType, includeForecasts }) => {
    ...
    try {
      ...
      return { subject, subjectType, historical, relatedReports, activeEvents, hasActiveEvents: activeEvents.length > 0, includeForecasts }
    } catch {
      return { subject, subjectType, error: 'Could not fetch data', includeForecasts }
    }
  },
})
...
const summarizeRecentReports = tool({
  ...
  execute: async ({ category, limit }) => {
    try {
      ...
      return await $fetch(`/api/reports?${query}`)
    } catch {
      return { reports: [], note: 'Report data unavailable' }
    }
  },
})
...
export default defineEventHandler(async (event) => {
  ...
  const ollamaBaseURL = `${process.env.OLLAMA_URL || 'http://localhost:11434'}/api`
  // Strip 'ollama:' prefix if present in env var
  const modelName = (process.env.OLLAMA_MODEL || 'llama3.2').replace(/^ollama:/, '')
