<template>
  ...
  >
    ...
    <Transition name="popup-fade">
      ...
      >
        <slot 
          ...
          :navigate="(r: GlobeRegion) => onNavigate(r)"
        >
          <!-- Default popup -->
          <GlobeRegionPopup
            :region="hoveredRegion"
    ...
    <!-- Zoom controls -->
    <div v-if="globeReady && enableZoom" class="zoom-controls">
...
// ── Slots ─────────────────────────────────────────────────────────────────────
defineSlots<{
  popup?: (props: { region: GlobeRegion; navigate: (region: GlobeRegion) => void }) => any
}>()
...
async function initGlobe() {
  ...
  globeInstance = Globe({ animateIn: false })(globeEl.value)
    ...
    .onPolygonHover((feat: GeoFeature | null) => {
      ...
      emit('regionHover', hoveredRegion.value)
    })
  ...
  // Enable zoom buttons only (no mouse wheel)
  globeInstance.controls().enableZoom = false // Disable mouse wheel
...
function zoomIn() {
  ...
  const newAltitude = Math.max(0.5, (current?.altitude || 2.2) - 0.4)
  globeInstance.pointOfView({ altitude: newAltitude }, 300)
}

function zoomOut() {
  if (!globeInstance) return
  const current = globeInstance.pointOfView()
  const newAltitude = (current?.altitude || 2.2) + 0.4
  globeInstance.pointOfView({ altitude: newAltitude }, 300)
}
