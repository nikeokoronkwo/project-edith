  ...
  <div class="detail-root">
    ...
    <div class="page-toolbar">
      <div class="breadcrumb-nav">
        ...
        <span class="bc-dim">SECTORS</span>
        <span class="bc-sep">/</span>
        <span class="bc-current">{{ formatSector(sectorId) }}</span>
      </div>
      <div class="toolbar-right">
        <div class="pill-nav">
          <NuxtLink
            ...
            :key="sec"
            :to="`/analytics/sector/${sec}`"
            class="nav-chip"
            :class="{ active: sec === sectorId }"
          >{{ formatSector(sec) }}</NuxtLink>
        </div>
    ...
    <div class="stats-row">
      ...
      >
        <span class="stat-name">{{ formatResource(resourceFromId(s.id)) }}</span>
    ...
    <div class="main-chart-area">
      <AnalyticsChart
        :series="seriesList"
        :title="`${formatSector(sectorId)} — ALL RESOURCES`"
...
const sectorId = computed(() => decodeURIComponent(route.params.id as string))

const { connected, hydrated, sectors, seriesForSector, hydrate } = useAnalyticsStream()

const seriesList = seriesForSector(sectorId)
